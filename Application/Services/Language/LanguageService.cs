...
namespace Application.Services.Language
{
	public class LanguageService : ILanguageService
	{
		...
		public async Task<ServiceResponse<LanguageDTO>> CreateLanguageAsync(CULanguageDTO cudLanguageDTO)
		{
			...
			try
			{
				var existingLanguages = await _unitOfWork.LanguageRepository.GetAllAsync();

				if (existingLanguages.Any(lang => lang.Name.Equals(cudLanguageDTO.Name, StringComparison.OrdinalIgnoreCase)))
				{
					response.Success = false;
					response.Message = "Ngôn ngữ đã tồn tại.";
					return response;
				}
		...
		public async Task<ServiceResponse<bool>> DeleteLanguageAsync(Guid id)
		{
			...
			try
			{
				...
				{
					response.Success = false;
					response.Message = "Lỗi khi cập nhật ngôn ngữ.";
				}
			}
			catch (Exception ex)
			{
		...
		public async Task<ServiceResponse<IEnumerable<LanguageDTO>>> GetAllLanguagesAsync()
		{
			...
			try
			{
				...
				var languageDTOs = _mapper.Map<List<LanguageDTO>>(languageList).OrderBy(x => x.Name)
						  .ToList();
